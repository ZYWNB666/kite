...
- **OAuth 集成**：支持多种 OAuth 提供商的单点登录
- **基于角色的访问控制 (RBAC)**：细粒度的权限管理
  - 资源级权限（集群、命名空间、资源类型、操作）
  - **资源名称过滤**：按名称限制对特定资源的访问（例如：只允许访问特定 Pod）
  - **代理权限**：控制 kite-proxy 客户端的 kubeconfig API 访问
  - **命名空间限制**：限制代理访问的命名空间范围
- **用户管理**：创建和管理用户及角色分配
- **API Key 认证**：支持程序化访问的 API Key
- **审计日志**：追踪所有用户操作和变更
